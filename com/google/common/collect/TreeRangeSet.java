...
/*  93 */       (this.asDescendingSetOfRanges = new AsRanges(this.rangesByLowerBound.descendingMap().values())) : result;
/*     */   }
/*     */   
...
/* 124 */     Map.Entry<Cut<C>, Range<C>> floorEntry = this.rangesByLowerBound.floorEntry(Cut.belowValue(value));
/* 125 */     if ((floorEntry != null) && (((Range)floorEntry.getValue()).contains(value))) {
/* 126 */       return (Range)floorEntry.getValue();
/*     */     }
/*     */     
/* 129 */     return null;
/*     */   }
...
/* 138 */       (((Range)ceilingEntry.getValue()).isConnected(range)) && 
...
/* 144 */       (((Range)priorEntry.getValue()).isConnected(range)) && 
...
/* 158 */     return (floorEntry != null) && (((Range)floorEntry.getValue()).encloses(range)) ? 
/* 159 */       (Range)floorEntry.getValue() : null;
/*     */   }
/*     */   
/*     */ 
/*     */   public Range<C> span()
...
/*     */     {
/* 189 */       Range<C> rangeBelowLB = (Range)entryBelowLB.getValue();
...
/*     */     {
/* 207 */       Range<C> rangeBelowUB = (Range)entryBelowUB.getValue();
...
/*     */     {
/* 234 */       Range<C> rangeBelowLB = (Range)entryBelowLB.getValue();
/* 235 */       if (rangeBelowLB.upperBound.compareTo(rangeToRemove.lowerBound) >= 0)
/*     */       {
/* 237 */         if ((rangeToRemove.hasUpperBound()) && 
/* 238 */           (rangeBelowLB.upperBound.compareTo(rangeToRemove.upperBound) >= 0))
/*     */         {
...
/*     */     {
/* 251 */       Range<C> rangeBelowUB = (Range)entryBelowUB.getValue();
/* 252 */       if ((rangeToRemove.hasUpperBound()) && 
/* 253 */         (rangeBelowUB.upperBound.compareTo(rangeToRemove.upperBound) >= 0))
/*     */       {
...
/* 349 */             return (Range)candidate.getValue();
/*     */           }
...
/* 370 */         Iterator<Range<C>> backingItr; if (lowerEntry == null) {
/* 371 */           backingItr = this.rangesByLowerBound.values().iterator(); } else { Iterator<Range<C>> backingItr;
/* 372 */           if (this.upperBoundWindow.lowerBound.isLessThan(((Range)lowerEntry.getValue()).upperBound)) {
...
/* 411 */       if ((backingItr.hasNext()) && 
/* 412 */         (this.upperBoundWindow.upperBound.isLessThan(((Range)backingItr.peek()).upperBound))) {
/* 413 */         backingItr.next();
/*     */       }
/* 415 */       new AbstractIterator()
/*     */       {
/*     */         protected Map.Entry<Cut<C>, Range<C>> computeNext() {
/* 418 */           if (!backingItr.hasNext()) {
/* 419 */             return (Map.Entry)endOfData();
/*     */           }
/* 421 */           Range<C> range = (Range)backingItr.next();
/* 422 */           return TreeRangeSet.RangesByUpperBound.this.upperBoundWindow.lowerBound.isLessThan(range.upperBound) ? 
/* 423 */             Maps.immutableEntry(range.upperBound, range) : 
/* 424 */             (Map.Entry)endOfData();
/*     */         }
/*     */       };
/*     */     }
/*     */     
...
/* 439 */       return 
/*     */       
/* 441 */         !entryIterator().hasNext() ? true : this.upperBoundWindow.equals(Range.all()) ? this.rangesByLowerBound.isEmpty() : false;
/*     */     }
/*     */   }
/*     */   
/*     */ 
...
/* 526 */       if ((this.complementLowerBoundWindow.contains(Cut.belowAll())) && (
/* 527 */         (!positiveItr.hasNext()) || (((Range)positiveItr.peek()).lowerBound != Cut.belowAll()))) {
/* 528 */         firstComplementRangeLowerBound = Cut.belowAll(); } else { Cut<C> firstComplementRangeLowerBound;
/* 529 */         if (positiveItr.hasNext()) {
...
/* 534 */       new AbstractIterator() {
/* 535 */         Cut<C> nextComplementRangeLowerBound = firstComplementRangeLowerBound;
/*     */         
/*     */         protected Map.Entry<Cut<C>, Range<C>> computeNext()
/*     */         {
/* 539 */           if ((TreeRangeSet.ComplementRangesByLowerBound.this.complementLowerBoundWindow.upperBound.isLessThan(this.nextComplementRangeLowerBound)) || 
/* 540 */             (this.nextComplementRangeLowerBound == Cut.aboveAll())) {
/* 541 */             return (Map.Entry)endOfData();
/*     */           }
/*     */           Range<C> negativeRange;
/* 544 */           if (positiveItr.hasNext()) {
/* 545 */             Range<C> positiveRange = (Range)positiveItr.next();
/* 546 */             Range<C> negativeRange = Range.create(this.nextComplementRangeLowerBound, positiveRange.lowerBound);
/* 547 */             this.nextComplementRangeLowerBound = positiveRange.upperBound;
/*     */           } else {
/* 549 */             negativeRange = Range.create(this.nextComplementRangeLowerBound, Cut.aboveAll());
...
/*     */     Iterator<Map.Entry<Cut<C>, Range<C>>> descendingEntryIterator()
/*     */     {
/* 570 */       Cut<C> startingPoint = this.complementLowerBoundWindow.hasUpperBound() ? (Cut)this.complementLowerBoundWindow.upperEndpoint() : Cut.aboveAll();
/*     */       
/*     */ 
...
/* 580 */         .iterator());
/*     */       Cut<C> cut;
/* 582 */       Cut<C> cut; if (positiveItr.hasNext())
/*     */       {
/*     */ 
/*     */ 
/* 586 */         cut = ((Range)positiveItr.peek()).upperBound == Cut.aboveAll() ? ((Range)positiveItr.next()).lowerBound : (Cut)this.positiveRangesByLowerBound.higherKey(((Range)positiveItr.peek()).upperBound);
/* 587 */       } else { if ((!this.complementLowerBoundWindow.contains(Cut.belowAll())) || 
/* 588 */           (this.positiveRangesByLowerBound.containsKey(Cut.belowAll()))) {
/* 589 */           return Iterators.emptyIterator();
/*     */         }
/* 591 */         cut = (Cut)this.positiveRangesByLowerBound.higherKey(Cut.belowAll());
/*     */       }
/*     */       
/* 594 */       final Cut<C> firstComplementRangeUpperBound = (Cut)MoreObjects.firstNonNull(cut, Cut.aboveAll());
/* 595 */       new AbstractIterator() {
...
/* 601 */             return (Map.Entry)endOfData();
/* 602 */           if (positiveItr.hasNext()) {
/* 603 */             Range<C> positiveRange = (Range)positiveItr.next();
/*     */             
...
/* 611 */             Range<C> negativeRange = Range.create(Cut.belowAll(), this.nextComplementRangeUpperBound);
/* 612 */             this.nextComplementRangeUpperBound = Cut.belowAll();
/* 613 */             return Maps.immutableEntry(Cut.belowAll(), negativeRange);
/*     */           }
/* 615 */           return (Map.Entry)endOfData();
/*     */         }
/*     */       };
/*     */     }
/*     */     
...
/* 634 */             return (Range)firstEntry.getValue();
/*     */           }
/*     */         } catch (ClassCastException e) {
/* 637 */           return null;
/*     */         }
...
/*     */     {
/* 666 */       return !TreeRangeSet.this.contains(value);
/*     */     }
/*     */     
/*     */     public RangeSet<C> complement()
/*     */     {
...
/*     */     {
/* 696 */       this.lowerBoundWindow = ((Range)Preconditions.checkNotNull(lowerBoundWindow));
/* 697 */       this.restriction = ((Range)Preconditions.checkNotNull(restriction));
/* 698 */       this.rangesByLowerBound = ((NavigableMap)Preconditions.checkNotNull(rangesByLowerBound));
...
/*     */         try
/*     */         {
/* 747 */           Cut<C> cut = (Cut)key;
/* 748 */           if ((!this.lowerBoundWindow.contains(cut)) || 
/* 749 */             (cut.compareTo(this.restriction.lowerBound) < 0) || 
/* 750 */             (cut.compareTo(this.restriction.upperBound) >= 0))
/* 751 */             return null;
/* 752 */           if (cut.equals(this.restriction.lowerBound))
/*     */           {
/* 754 */             Range<C> candidate = (Range)Maps.valueOrNull(this.rangesByLowerBound.floorEntry(cut));
...
/*     */             }
/*     */           } else {
/* 759 */             Range<C> result = (Range)this.rangesByLowerBound.get(cut);
/* 760 */             if (result != null) {
...
/* 779 */       Iterator<Range<C>> completeRangeItr; final Iterator<Range<C>> completeRangeItr; if (this.lowerBoundWindow.lowerBound.isLessThan(this.restriction.lowerBound))
/*     */       {
/*     */ 
...
/*     */       
/*     */ 
/* 795 */       final Cut<Cut<C>> upperBoundOnLowerBounds = (Cut)Ordering.natural().min(this.lowerBoundWindow.upperBound, Cut.belowValue(this.restriction.upperBound));
/* 796 */       new AbstractIterator()
/*     */       {
/*     */         protected Map.Entry<Cut<C>, Range<C>> computeNext() {
/* 799 */           if (!completeRangeItr.hasNext()) {
/* 800 */             return (Map.Entry)endOfData();
/*     */           }
/* 802 */           Range<C> nextRange = (Range)completeRangeItr.next();
/* 803 */           if (upperBoundOnLowerBounds.isLessThan(nextRange.lowerBound)) {
/* 804 */             return (Map.Entry)endOfData();
/*     */           }
...
/*     */       
/*     */ 
/* 820 */       Cut<Cut<C>> upperBoundOnLowerBounds = (Cut)Ordering.natural().min(this.lowerBoundWindow.upperBound, Cut.belowValue(this.restriction.upperBound));
/*     */       
/*     */ 
/*     */ 
/*     */ 
...
/*     */           }
/* 843 */           return (Map.Entry)endOfData();
/*     */         }
/*     */       };
/*     */     }
/*     */     
...
/* 891 */         this.restriction.encloses(rangeToAdd), "Cannot add range %s to subRangeSet(%s)", rangeToAdd, this.restriction);
/*     */       
/*     */ 
/*     */ 
...
/*     */     public boolean contains(C value)
/*     */     {
/* 907 */       return (this.restriction.contains(value)) && (TreeRangeSet.this.contains(value));
/*     */     }
/*     */     
/*     */     public void clear()
/*     */     {
...
/* 920 */         return new SubRangeSet(this, this.restriction.intersection(view));
/*     */       }
/* 922 */       return ImmutableRangeSet.of();
/*     */     }
/*     */   }
/*     */ }


/* Location:              C:\Users\Lab\Desktop\a.jar!\com\google\common\collect\TreeRangeSet.class
