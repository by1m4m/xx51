...
/*     */     
/*  76 */     for (Map.Entry<Class<?>, Collection<Subscriber>> entry : listenerMethods.asMap().entrySet()) {
/*  77 */       Class<?> eventType = (Class)entry.getKey();
/*  78 */       Collection<Subscriber> eventMethodsInListener = (Collection)entry.getValue();
/*     */       
/*  80 */       CopyOnWriteArraySet<Subscriber> eventSubscribers = (CopyOnWriteArraySet)this.subscribers.get(eventType);
/*     */       
/*  82 */       if (eventSubscribers == null) {
/*  83 */         CopyOnWriteArraySet<Subscriber> newSet = new CopyOnWriteArraySet();
/*     */         
/*  85 */         eventSubscribers = (CopyOnWriteArraySet)MoreObjects.firstNonNull(this.subscribers.putIfAbsent(eventType, newSet), newSet);
/*     */       }
/*     */       
...
/*     */     
/*  96 */     for (Map.Entry<Class<?>, Collection<Subscriber>> entry : listenerMethods.asMap().entrySet()) {
/*  97 */       Class<?> eventType = (Class)entry.getKey();
/*  98 */       Collection<Subscriber> listenerMethodsForType = (Collection)entry.getValue();
/*     */       
/* 100 */       CopyOnWriteArraySet<Subscriber> currentSubscribers = (CopyOnWriteArraySet)this.subscribers.get(eventType);
...
/*     */   {
/* 117 */     return (Set)MoreObjects.firstNonNull(this.subscribers.get(eventType), ImmutableSet.of());
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */     
/* 130 */     for (UnmodifiableIterator localUnmodifiableIterator = eventTypes.iterator(); localUnmodifiableIterator.hasNext();) { Class<?> eventType = (Class)localUnmodifiableIterator.next();
/* 131 */       CopyOnWriteArraySet<Subscriber> eventSubscribers = (CopyOnWriteArraySet)this.subscribers.get(eventType);
...
/*     */   {
/* 162 */     Multimap<Class<?>, Subscriber> methodsInListener = HashMultimap.create();
/* 163 */     Class<?> clazz = listener.getClass();
/* 164 */     for (UnmodifiableIterator localUnmodifiableIterator = getAnnotatedMethods(clazz).iterator(); localUnmodifiableIterator.hasNext();) { Method method = (Method)localUnmodifiableIterator.next();
/* 165 */       Class<?>[] parameterTypes = method.getParameterTypes();
/* 166 */       Class<?> eventType = parameterTypes[0];
/* 167 */       methodsInListener.put(eventType, Subscriber.create(this.bus, listener, method));
/*     */     }
/* 169 */     return methodsInListener;
/*     */   }
/*     */   
/*     */   private static ImmutableList<Method> getAnnotatedMethods(Class<?> clazz) {
/* 173 */     return (ImmutableList)subscriberMethodsCache.getUnchecked(clazz);
/*     */   }
/*     */   
...
/* 181 */         if ((method.isAnnotationPresent(Subscribe.class)) && (!method.isSynthetic()))
/*     */         {
...
/*     */     try
/*     */     {
/* 223 */       return (ImmutableSet)flattenHierarchyCache.getUnchecked(concreteClass);
...
/* Location:              C:\Users\Lab\Desktop\a.jar!\com\google\common\eventbus\SubscriberRegistry.class
