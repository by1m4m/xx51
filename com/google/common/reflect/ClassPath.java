...
/*     */   
/*  81 */   private static final Predicate<ClassInfo> IS_TOP_LEVEL = new Predicate()
/*     */   {
...
/*     */   {
/* 137 */     return FluentIterable.from(this.resources).filter(ClassInfo.class).toSet();
/*     */   }
/*     */   
/*     */   public ImmutableSet<ClassInfo> getTopLevelClasses()
/*     */   {
/* 142 */     return FluentIterable.from(this.resources).filter(ClassInfo.class).filter(IS_TOP_LEVEL).toSet();
/*     */   }
/*     */   
...
/* 149 */     for (UnmodifiableIterator localUnmodifiableIterator = getTopLevelClasses().iterator(); localUnmodifiableIterator.hasNext();) { ClassInfo classInfo = (ClassInfo)localUnmodifiableIterator.next();
...
/* 165 */     for (UnmodifiableIterator localUnmodifiableIterator = getTopLevelClasses().iterator(); localUnmodifiableIterator.hasNext();) { ClassInfo classInfo = (ClassInfo)localUnmodifiableIterator.next();
...
/* 353 */       for (UnmodifiableIterator localUnmodifiableIterator = getClassPathEntries(classloader).entrySet().iterator(); localUnmodifiableIterator.hasNext();) { Map.Entry<File, ClassLoader> entry = (Map.Entry)localUnmodifiableIterator.next();
/* 354 */         scan((File)entry.getKey(), (ClassLoader)entry.getValue());
/*     */       }
/*     */     }
/*     */     
/*     */     @VisibleForTesting
...
/*     */     }
/*     */     
/*     */     protected abstract void scanDirectory(ClassLoader paramClassLoader, File paramFile) throws IOException;
/*     */     
...
/*     */       }
/*     */       try { JarFile jarFile;
/* 397 */         for (e = getClassPathFromManifest(file, jarFile.getManifest()).iterator(); e.hasNext();) { File path = (File)e.next();
/* 398 */           scan(path, classloader);
/*     */         }
/* 400 */         scanJarFile(classloader, jarFile); return;
/*     */       } finally {
/*     */         try {
/* 403 */           jarFile.close();
/*     */         }
...
/*     */       }
/* 450 */       for (UnmodifiableIterator localUnmodifiableIterator = getClassLoaderUrls(classloader).iterator(); localUnmodifiableIterator.hasNext();) { URL url = (URL)localUnmodifiableIterator.next();
...
/* 511 */       for (Map.Entry<ClassLoader, String> entry : this.resources.entries()) {
/* 512 */         builder.add(ClassPath.ResourceInfo.of((String)entry.getValue(), (ClassLoader)entry.getKey()));
/*     */       }
...
/* 521 */         JarEntry entry = (JarEntry)entries.nextElement();
...
/* Location:              C:\Users\Lab\Desktop\a.jar!\com\google\common\reflect\ClassPath.class
