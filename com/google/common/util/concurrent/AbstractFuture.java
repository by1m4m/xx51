...
/*      */ 
/*      */ 
/*      */   static abstract interface Trusted<V>
...
/*      */     {
/*   97 */       return (V)super.get(timeout, unit);
/*      */     }
/*      */     
...
/*      */     AtomicHelper helper;
/*      */     try {
/*  137 */       helper = new UnsafeAtomicHelper(null);
/*      */     } catch (Throwable unsafeFailure) { AtomicHelper helper;
...
/*  157 */         helper = new SynchronizedHelper(null);
/*      */       }
/*      */     }
...
/*      */     }
/*      */     for (;;) {
/*  233 */       if (curr == null) return;
/*  234 */       Waiter succ = curr.next;
/*  235 */       if (curr.thread != null) {
/*  236 */         pred = curr;
/*  237 */       } else { if (pred != null) {
/*  238 */           pred.next = succ;
/*  239 */           if (pred.thread != null) break label78;
/*  240 */           break;
/*      */         }
/*  242 */         if (!ATOMIC_HELPER.casWaiters(this, curr, succ)) break;
/*      */       }
/*      */       label78:
/*  245 */       curr = succ;
/*      */     }
...
/*  407 */       return (V)getDoneValue(localValue);
/*      */     }
/*      */     
...
/*  431 */                 return (V)getDoneValue(localValue);
/*      */               }
/*      */               
/*      */ 
...
/*      */       
/*      */ 
/*  448 */       return (V)getDoneValue(this.value);
/*      */     }
/*      */     
/*      */ 
...
/*  455 */         return (V)getDoneValue(localValue);
/*      */       }
...
/*  513 */       return (V)getDoneValue(localValue);
/*      */     }
...
/*  532 */           } while (((localValue != null ? 1 : 0) & (!(localValue instanceof SetFuture) ? 1 : 0)) == 0);
/*  533 */           return (V)getDoneValue(localValue);
/*      */         }
/*      */         
/*      */ 
...
/*      */     
/*      */ 
/*  542 */     return (V)getDoneValue(this.value);
/*      */   }
/*      */   
/*      */ 
...
/*  565 */     return (localValue != null ? 1 : 0) & (!(localValue instanceof SetFuture) ? 1 : 0);
/*      */   }
/*      */   
...
/*      */     {
/*      */ 
/*  595 */       Object valueToSet = mayInterruptIfRunning ? Cancellation.CAUSELESS_INTERRUPTED : GENERATE_CANCELLATION_CAUSES ? new Cancellation(mayInterruptIfRunning, new CancellationException("Future.cancel() was called.")) : Cancellation.CAUSELESS_CANCELLED;
/*      */       
/*      */ 
/*      */ 
/*      */ 
...
/*  792 */         return false;
/*      */       }
/*  794 */       SetFuture valueToSet = new SetFuture(this, future);
...
/*      */           }
/*      */           
/*  811 */           oomMostLikely = ATOMIC_HELPER.casValue(this, valueToSet, failure);
/*      */         }
/*  813 */         return true;
/*      */       }
...
/*      */     try
/*      */     {
/*  911 */       return (V)future.get();
/*      */     } catch (InterruptedException e) {
/*  913 */       for (;;) { interrupted = true;
/*      */       }
/*      */     }
/*      */     finally {
/*  917 */       if (interrupted) {
...
/*  937 */     future = null;
/*  938 */     label100: for (;;) { if (next == null) return;
...
/* 1100 */       return 
/* 1101 */         "remaining delay=[" + ((ScheduledFuture)this).getDelay(TimeUnit.MILLISECONDS) + " ms]";
/*      */     }
/*      */     
/* 1104 */     return null;
/*      */   }
...
/*      */         try {
/* 1188 */           unsafe = (Unsafe)AccessController.doPrivileged(new PrivilegedExceptionAction()
/*      */           {
...
/*      */     }
/*      */     
/* 1306 */     void putThread(AbstractFuture.Waiter waiter, Thread newValue) { waiter.thread = newValue; }
/*      */     
/*      */ 
/*      */     void putNext(AbstractFuture.Waiter waiter, AbstractFuture.Waiter newValue)
/*      */     {
...
/* Location:              C:\Users\Lab\Desktop\a.jar!\com\google\common\util\concurrent\AbstractFuture.class
