...
/*     */   {
/* 109 */     Channel c = (Channel)this.nonServerChannels.get(id);
/* 110 */     if (c != null) {
/* 111 */       return c;
/*     */     }
/* 113 */     return (Channel)this.serverChannels.get(id);
/*     */   }
/*     */   
/*     */ 
...
/*     */   {
/* 139 */     ConcurrentMap<ChannelId, Channel> map = (channel instanceof ServerChannel) ? this.serverChannels : this.nonServerChannels;
/*     */     
/*     */ 
/* 142 */     boolean added = map.putIfAbsent(channel.id(), channel) == null;
/* 143 */     if (added) {
/* 144 */       channel.closeFuture().addListener(this.remover);
/*     */     }
/*     */     
...
/* 170 */       c = (Channel)this.nonServerChannels.remove(o);
/* 171 */       if (c == null) {
/* 172 */         c = (Channel)this.serverChannels.remove(o);
/*     */       }
/* 174 */     } else if ((o instanceof Channel)) {
/* 175 */       c = (Channel)o;
/* 176 */       if ((c instanceof ServerChannel)) {
/* 177 */         c = (Channel)this.serverChannels.remove(c.id());
/*     */       } else {
/* 179 */         c = (Channel)this.nonServerChannels.remove(c.id());
/*     */       }
/*     */     }
/*     */     
/* 183 */     if (c == null) {
...
/*     */     }
/*     */     
/* 187 */     c.closeFuture().removeListener(this.remover);
/* 188 */     return true;
/*     */   }
/*     */   
/*     */   public void clear()
...
/*     */   {
/* 199 */     return new CombinedIterator(this.serverChannels
...
/*     */   {
/* 206 */     Collection<Channel> channels = new ArrayList(size());
...
/*     */   {
/* 214 */     Collection<Channel> channels = new ArrayList(size());
...
/*     */     ChannelGroupFuture future;
/* 267 */     if (voidPromise) {
/* 268 */       for (Iterator localIterator = this.nonServerChannels.values().iterator(); localIterator.hasNext();) { c = (Channel)localIterator.next();
...
/*     */     } else {
/* 275 */       Object futures = new LinkedHashMap(size());
/* 276 */       for (Channel c : this.nonServerChannels.values()) {
/* 277 */         if (matcher.matches(c)) {
/* 278 */           ((Map)futures).put(c, c.write(safeDuplicate(message)));
/*     */         }
/*     */       }
/* 281 */       future = new DefaultChannelGroupFuture(this, (Map)futures, this.executor);
/*     */     }
...
/*     */     
/*     */ 
/* 309 */     Map<Channel, ChannelFuture> futures = new LinkedHashMap(size());
/*     */     
...
/*     */     
/*     */ 
/* 332 */     Map<Channel, ChannelFuture> futures = new LinkedHashMap(size());
/*     */     
...
/*     */     
/*     */ 
/* 365 */     Map<Channel, ChannelFuture> futures = new LinkedHashMap(size());
/*     */     
...
/*     */     ChannelGroupFuture future;
/* 408 */     if (voidPromise) {
/* 409 */       for (Iterator localIterator = this.nonServerChannels.values().iterator(); localIterator.hasNext();) { c = (Channel)localIterator.next();
...
/*     */     } else {
/* 416 */       Object futures = new LinkedHashMap(size());
/* 417 */       for (Channel c : this.nonServerChannels.values()) {
/* 418 */         if (matcher.matches(c)) {
/* 419 */           ((Map)futures).put(c, c.writeAndFlush(safeDuplicate(message)));
/*     */         }
/*     */       }
/* 422 */       future = new DefaultChannelGroupFuture(this, (Map)futures, this.executor);
/*     */     }
...
/*     */   {
/* 436 */     Map<Channel, ChannelFuture> futures = new LinkedHashMap(size());
/*     */     
