...
/* 149 */     this.config = new NioDatagramChannelConfig(this, socket);
/*     */   }
/*     */   
...
/*     */   {
/* 165 */     java.nio.channels.DatagramChannel ch = javaChannel();
/* 166 */     return (ch.isOpen()) && (
/* 167 */       ((((Boolean)this.config.getOption(ChannelOption.DATAGRAM_CHANNEL_ACTIVE_ON_REGISTRATION)).booleanValue()) && (isRegistered())) || 
...
/*     */   {
/* 178 */     return (java.nio.channels.DatagramChannel)super.javaChannel();
/*     */   }
/*     */   
...
/*     */   {
/* 240 */     java.nio.channels.DatagramChannel ch = javaChannel();
...
/*     */     {
/* 276 */       AddressedEnvelope<ByteBuf, SocketAddress> envelope = (AddressedEnvelope)msg;
/* 277 */       SocketAddress remoteAddress = envelope.recipient();
...
/*     */     int writtenBytes;
/* 292 */     int writtenBytes; if (remoteAddress != null) {
...
/* 306 */         return p;
/*     */       }
/* 308 */       return new DatagramPacket(newDirectBuffer(p, content), (InetSocketAddress)p.recipient());
/*     */     }
/*     */     
...
/*     */     {
/* 321 */       AddressedEnvelope<Object, SocketAddress> e = (AddressedEnvelope)msg;
/* 322 */       if ((e.content() instanceof ByteBuf)) {
/* 323 */         ByteBuf content = (ByteBuf)e.content();
/* 324 */         if (isSingleDirectBuffer(content)) {
/* 325 */           return e;
/*     */         }
/* 327 */         return new DefaultAddressedEnvelope(newDirectBuffer(e, content), e.recipient());
/*     */       }
/*     */     }
/*     */     
/*     */ 
/* 332 */     throw new UnsupportedOperationException("unsupported message type: " + StringUtil.simpleClassName(msg) + EXPECTED_TYPES);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */       
/* 371 */         NetworkInterface.getByInetAddress(localAddress().getAddress()), null, promise);
/*     */     }
...
/* 424 */           this.memberships = new HashMap();
/*     */         } else {
/* 426 */           keys = (List)this.memberships.get(multicastAddress);
/*     */         }
/* 428 */         if (keys == null) {
/* 429 */           keys = new ArrayList();
...
/* 452 */         NetworkInterface.getByInetAddress(localAddress().getAddress()), null, promise);
/*     */     } catch (SocketException e) {
...
/* 493 */         List<MembershipKey> keys = (List)this.memberships.get(multicastAddress);
/* 494 */         if (keys != null) {
/* 495 */           Iterator<MembershipKey> keyIt = keys.iterator();
/*     */           
/* 497 */           while (keyIt.hasNext()) {
/* 498 */             MembershipKey key = (MembershipKey)keyIt.next();
/* 499 */             if ((networkInterface.equals(key.networkInterface())) && (
/* 500 */               ((source == null) && (key.sourceAddress() == null)) || ((source != null) && 
/* 501 */               (source.equals(key.sourceAddress()))))) {
/* 502 */               key.drop();
/* 503 */               keyIt.remove();
/*     */             }
/*     */           }
...
/* 549 */         List<MembershipKey> keys = (List)this.memberships.get(multicastAddress);
