...
/* 281 */       int toRead = Math.min(buffer.readableBytes(), this.maxChunkSize);
/* 282 */       if (toRead > 0) {
...
/*     */       }
/*     */       
/* 301 */       int toRead = Math.min(readLimit, this.maxChunkSize);
/* 302 */       if (toRead > this.chunkSize) {
/* 303 */         toRead = (int)this.chunkSize;
/*     */       }
/* 305 */       ByteBuf content = buffer.readRetainedSlice(toRead);
/* 306 */       this.chunkSize -= toRead;
/*     */       
...
/* 340 */       int toRead = Math.min((int)this.chunkSize, this.maxChunkSize);
...
/* 357 */       int wIdx = buffer.writerIndex();
...
/* 411 */       boolean chunked = HttpUtil.isTransferEncodingChunked(this.message);
...
/*     */       {
/*     */ 
/* 422 */         out.add(invalidMessage(Unpooled.EMPTY_BUFFER, new PrematureChannelClosureException("Connection closed before received headers")));
/*     */         
/* 424 */         resetNow(); return;
/*     */       }
/*     */       
...
/* 620 */       nextState = State.SKIP_CONTROL_CHARS; } else { State nextState;
/* 621 */       if (HttpUtil.isTransferEncodingChunked(message)) {
/* 622 */         nextState = State.READ_CHUNK_SIZE; } else { State nextState;
/* 623 */         if (contentLength() >= 0L) {
...
/* 658 */             String currentLastPos = (String)current.get(lastPos);
...
/*     */   
/* 693 */   private static int getChunkSize(String hex) { hex = hex.trim();
...
/*     */ 
/*     */ 
/* 736 */     int nameStart = findNonWhitespace(sb, 0);
/* 737 */     for (int nameEnd = nameStart; nameEnd < length; nameEnd++) {
...
/*     */     }
/*     */     
/* 744 */     for (int colonEnd = nameEnd; colonEnd < length; colonEnd++) {
