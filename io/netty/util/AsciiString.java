...
/*      */     {
/*  136 */       throw new IndexOutOfBoundsException("expected: 0 <= start(" + start + ") <= start + length(" + length + ") <= value.capacity(" + value.capacity() + ')');
/*      */     }
/*      */     
...
/*  176 */     int i = 0; for (int j = start; i < length; j++) {
/*  177 */       this.value[i] = c2b(value[j]);i++;
/*      */     }
/*  179 */     this.offset = 0;
...
/*      */     {
/*  219 */       throw new IndexOutOfBoundsException("expected: 0 <= start(" + start + ") <= start + length(" + length + ") <= value.length(" + value.length() + ')');
/*      */     }
/*      */     
/*  222 */     this.value = new byte[length];
/*  223 */     int i = 0; for (int j = start; i < length; j++) {
/*  224 */       this.value[i] = c2b(value.charAt(j));i++;
/*      */     }
/*  226 */     this.offset = 0;
...
/*      */     {
/*  272 */       throw new IndexOutOfBoundsException("expected: 0 <= index(" + index + ") <= start + length(" + length + ") <= length(" + length() + ')');
/*      */     }
...
/*      */     {
/*  307 */       throw new IndexOutOfBoundsException("expected: 0 <= index(" + index + ") <= start + length(" + length + ") <= length(" + length() + ')');
/*      */     }
...
/*      */     {
/*  413 */       throw new IndexOutOfBoundsException("expected: 0 <= srcIdx(" + srcIdx + ") <= srcIdx + length(" + length + ") <= srcLen(" + length() + ')');
/*      */     }
/*      */     
...
/*  457 */     int i = 0; for (int j = arrayOffset(); i < minLength; j++) {
...
/*  487 */       System.arraycopy(this.value, arrayOffset(), newValue, 0, thisLen);
...
/*  498 */     int i = thisLen; for (int j = 0; i < newValue.length; j++) {
/*  499 */       newValue[i] = c2b(string.charAt(j));i++;
/*      */     }
/*      */     
...
/*  531 */       int i = arrayOffset(); for (int j = rhs.arrayOffset(); i < length(); j++) {
/*  532 */         if (!equalsIgnoreCase(this.value[i], rhs.value[j])) {
/*  533 */           return false;
/*      */         }
/*  531 */         i++;
/*      */       }
...
/*      */     }
/*      */     
/*  539 */     int i = arrayOffset(); for (int j = 0; i < length(); j++) {
...
/*      */     {
/*  569 */       throw new IndexOutOfBoundsException("expected: 0 <= start(" + start + ") <= srcIdx + length(" + length + ") <= srcLen(" + length() + ')');
/*      */     }
/*      */     
/*  572 */     char[] buffer = new char[length];
/*  573 */     int i = 0; for (int j = start + arrayOffset(); i < length; j++) {
/*  574 */       buffer[i] = b2c(this.value[j]);i++;
/*      */     }
/*  576 */     return buffer;
/*      */   }
/*      */   
...
/*      */     {
/*  594 */       throw new IndexOutOfBoundsException("expected: 0 <= srcIdx(" + srcIdx + ") <= srcIdx + length(" + length + ") <= srcLen(" + length() + ')');
/*      */     }
/*      */     
/*  597 */     int dstEnd = dstIdx + length;
/*  598 */     int i = dstIdx; for (int j = srcIdx + arrayOffset(); i < dstEnd; j++) {
/*  599 */       dst[i] = b2c(this.value[j]);i++;
/*      */     }
/*      */   }
/*      */   
/*      */ 
...
/*      */     {
/*  637 */       throw new IndexOutOfBoundsException("expected: 0 <= start(" + start + ") <= end (" + end + ") <= length(" + length() + ')');
/*      */     }
/*      */     
...
/*  694 */         int o1 = i;int o2 = 0;
/*  695 */         do { o2++; } while ((o2 < subCount) && (b2c(this.value[(++o1)]) == subString.charAt(o2)));
/*      */         
/*      */ 
/*  698 */         if (o2 == subCount) {
...
/*  725 */     int len = this.offset + this.length;
...
/*  778 */         int o1 = i;int o2 = 0;
/*  779 */         do { o2++; } while ((o2 < subCount) && (b2c(this.value[(++o1)]) == subString.charAt(o2)));
/*      */         
/*      */ 
/*  782 */         if (o2 == subCount) {
...
/*  820 */     int i = start; for (int j = thisStart + arrayOffset(); i < thatEnd; j++) {
...
/*      */     
/*  931 */     int len = length() + arrayOffset();
/*  932 */     for (int i = arrayOffset(); i < len; i++) {
...
/*      */     }
/*      */     
/*  945 */     byte[] newValue = new byte[length()];
/*  946 */     i = 0; for (int j = arrayOffset(); i < newValue.length; j++) {
/*  947 */       newValue[i] = toLowerCase(this.value[j]);i++;
/*      */     }
/*      */     
...
/*      */     
/*  961 */     int len = length() + arrayOffset();
/*  962 */     for (int i = arrayOffset(); i < len; i++) {
...
/*      */     }
/*      */     
/*  975 */     byte[] newValue = new byte[length()];
/*  976 */     i = 0; for (int j = arrayOffset(); i < newValue.length; j++) {
/*  977 */       newValue[i] = toUpperCase(this.value[j]);i++;
/*      */     }
/*      */     
...
/*      */     }
/*  997 */     int start = 0;int last = c.length() - 1;
/*  998 */     int end = last;
...
/*      */   {
/* 1018 */     int start = arrayOffset();int last = arrayOffset() + length() - 1;
/* 1019 */     int end = last;
...
/*      */     }
/*      */     
/* 1046 */     int i = arrayOffset(); for (int j = 0; j < a.length(); j++) {
...
/* 1109 */         if (!((AsciiString)res.get(i)).isEmpty()) break;
/* 1110 */         res.remove(i);
/*      */       }
/*      */     }
/*      */     
...
/*      */ 
/*      */ 
/* 1118 */     return (AsciiString[])res.toArray(new AsciiString[0]);
/*      */   }
/*      */   
/*      */ 
/*      */ 
...
/*      */     {
/* 1184 */       throw new IndexOutOfBoundsException("expected: 0 <= start(" + start + ") <= srcIdx + length(" + length + ") <= srcLen(" + length() + ')');
/*      */     }
/*      */     
/*      */ 
...
/* 1206 */     return (char)(b2c(this.value[startWithOffset]) << '\b' | b2c(this.value[(startWithOffset + 1)]));
/*      */   }
/*      */   
...
/*      */   }
/*      */   
/* 1360 */   public static final HashingStrategy<CharSequence> CASE_INSENSITIVE_HASHER = new HashingStrategy()
/*      */   {
...
/*      */   };
/*      */   
/* 1373 */   public static final HashingStrategy<CharSequence> CASE_SENSITIVE_HASHER = new HashingStrategy()
/*      */   {
/*      */     public int hashCode(CharSequence o)
/*      */     {
...
/* 1453 */       return false;
/*      */     }
/* 1455 */     int i = 0; for (int j = 0; i < a.length(); j++) {
...
/*      */       {
/* 1583 */         bStart++; if (bStart == b.length())
/* 1584 */           return true;
/*      */       } else {
...
/* 1831 */     return isUpperCase(c) ? (char)(c + ' ') : c;
/*      */   }
/*      */   
