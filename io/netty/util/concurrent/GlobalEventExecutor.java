...
/*     */   
/*  45 */   final BlockingQueue<Runnable> taskQueue = new LinkedBlockingQueue();
/*  46 */   final ScheduledFutureTask<Void> quietPeriodTask = new ScheduledFutureTask(this, 
/*  47 */     Executors.callable(new Runnable() { public void run() {}
/*  47 */   }, null), 
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*  52 */     ScheduledFutureTask.deadlineNanos(SCHEDULE_QUIET_PERIOD_INTERVAL), -SCHEDULE_QUIET_PERIOD_INTERVAL);
/*     */   
/*     */ 
/*     */ 
/*     */ 
...
/*     */         try {
/*  82 */           task = (Runnable)taskQueue.take();
/*     */         }
...
/*  90 */       if (delayNanos > 0L) {
/*     */         try {
/*  92 */           task = (Runnable)taskQueue.poll(delayNanos, TimeUnit.NANOSECONDS);
/*     */         } catch (InterruptedException e) {
/*     */           Runnable task;
/*  95 */           return null;
/*     */         }
/*     */       } else {
/*  98 */         task = (Runnable)taskQueue.poll();
/*     */       }
/*     */       
/* 101 */       if (task == null) {
/* 102 */         fetchFromScheduledTaskQueue();
/* 103 */         task = (Runnable)taskQueue.poll();
/*     */       }
/*     */       
/* 106 */       if (task != null) {
/* 107 */         return task;
...
/*     */     
/*     */     public void run() {
/* 245 */       for (;;) { Runnable task = GlobalEventExecutor.this.takeTask();
/* 246 */         if (task != null) {
/*     */           try {
/* 248 */             task.run();
/*     */           } catch (Throwable t) {
...
/*     */           
/* 260 */           if ((GlobalEventExecutor.this.taskQueue.isEmpty()) && ((scheduledTaskQueue == null) || (scheduledTaskQueue.size() == 1)))
/*     */           {
/*     */ 
/*     */ 
...
/*     */             
/*     */ 
/* 268 */             if ((GlobalEventExecutor.this.taskQueue.isEmpty()) && ((scheduledTaskQueue == null) || (scheduledTaskQueue.size() == 1))) {
/*     */               break;
/*     */             }
/*     */             
/*     */ 
