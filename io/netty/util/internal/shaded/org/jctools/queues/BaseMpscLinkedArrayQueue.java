...
/*     */   {
/* 215 */     long after = lvConsumerIndex();
/*     */     
/*     */     for (;;)
/*     */     {
/* 219 */       long before = after;
/* 220 */       long currentProducerIndex = lvProducerIndex();
/* 221 */       after = lvConsumerIndex();
/* 222 */       if (before == after)
/*     */       {
/* 224 */         long size = currentProducerIndex - after >> 1;
/* 225 */         break;
/*     */       }
/*     */     }
/*     */     
...
/* 309 */     UnsafeRefArrayAccess.soElement(buffer, offset, e);
/* 310 */     return true;
/*     */   }
/*     */   
/*     */ 
...
/* 327 */     Object e = UnsafeRefArrayAccess.lvElement(buffer, offset);
/* 328 */     if (e == null)
/*     */     {
...
/*     */ 
/*     */ 
/* 337 */           e = UnsafeRefArrayAccess.lvElement(buffer, offset);
/*     */         }
/* 339 */         while (e == null);
/*     */       }
/*     */       else
...
/* 350 */       return (E)newBufferPoll(nextBuffer, index);
/*     */     }
/*     */     
/* 353 */     UnsafeRefArrayAccess.soElement(buffer, offset, null);
...
/* 372 */     Object e = UnsafeRefArrayAccess.lvElement(buffer, offset);
...
/*     */       {
/*     */ 
/* 379 */         e = UnsafeRefArrayAccess.lvElement(buffer, offset);
/*     */       }
/* 381 */       while (e == null);
/*     */     }
/* 383 */     if (e == JUMP)
/*     */     {
/* 385 */       return (E)newBufferPeek(getNextBuffer(buffer, mask), index);
/*     */     }
/* 387 */     return (E)e;
/*     */   }
/*     */   
...
/* 439 */     E[] nextBuffer = (Object[])UnsafeRefArrayAccess.lvElement(buffer, offset);
/* 440 */     UnsafeRefArrayAccess.soElement(buffer, offset, null);
...
/* 452 */     E n = UnsafeRefArrayAccess.lvElement(nextBuffer, offset);
/* 453 */     if (n == null)
/*     */     {
/* 455 */       throw new IllegalStateException("new buffer must have at least one element");
/*     */     }
/* 457 */     UnsafeRefArrayAccess.soElement(nextBuffer, offset, null);
...
/* 465 */     E n = UnsafeRefArrayAccess.lvElement(nextBuffer, offset);
/* 466 */     if (null == n)
/*     */     {
...
/* 476 */     this.consumerMask = (LinkedArrayQueueUtil.length(nextBuffer) - 2 << 1);
...
/* 510 */     Object e = UnsafeRefArrayAccess.lvElement(buffer, offset);
/* 511 */     if (e == null)
/*     */     {
/* 513 */       return null;
/*     */     }
...
/* 518 */       return (E)newBufferPoll(nextBuffer, index);
/*     */     }
/* 520 */     UnsafeRefArrayAccess.soElement(buffer, offset, null);
...
/* 534 */     Object e = UnsafeRefArrayAccess.lvElement(buffer, offset);
/* 535 */     if (e == JUMP)
/*     */     {
/* 537 */       return (E)newBufferPeek(getNextBuffer(buffer, mask), index);
/*     */     }
/* 539 */     return (E)e;
/*     */   }
/*     */   
...
/*     */ 
/*     */ 
/* 586 */         long batchIndex = Math.min(producerLimit, pIndex + 2 * batchSize);
/*     */         int result;
/* 588 */         if ((pIndex >= producerLimit) || (producerLimit < batchIndex))
/*     */         {
/* 590 */           result = offerSlowPath(mask, pIndex, producerLimit); }
/* 591 */         switch (result)
/*     */         {
/*     */         case 0: 
/*     */         case 1: 
...
/* 616 */       UnsafeRefArrayAccess.soElement(buffer, offset, s.get());
/*     */     }
...
/* 652 */     for (int i = 0; 
/*     */         
/* 654 */         (i < limit) && ((m = relaxedPoll()) != null); i++)
/*     */     {
/* 656 */       c.accept(m);
/*     */     }
/* 658 */     return i;
...
/*     */     
/* 690 */     UnsafeRefArrayAccess.soElement(newBuffer, offsetInNew, e);
/* 691 */     UnsafeRefArrayAccess.soElement(oldBuffer, nextArrayOffset(oldMask), newBuffer);
/*     */     
/*     */ 
...
/*     */ 
/*     */ 
/* 708 */     UnsafeRefArrayAccess.soElement(oldBuffer, offsetInOld, JUMP);
/*     */   }
/*     */   
