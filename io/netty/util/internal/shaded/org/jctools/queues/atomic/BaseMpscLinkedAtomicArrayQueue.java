...
/* 198 */       if (before == after) {
/* 199 */         long size = currentProducerIndex - after >> 1;
/* 200 */         break;
/*     */       }
/*     */     }
/*     */     long size;
...
/* 292 */       return (E)newBufferPoll(nextBuffer, index);
/*     */     }
...
/* 313 */     while ((e == null) && (index != lvProducerIndex()) && 
/*     */     
/* 315 */       ((e = LinkedAtomicArrayQueueUtil.lvElement(buffer, offset)) == null)) {}
/*     */     
/*     */ 
/* 318 */     if (e == JUMP) {
/* 319 */       return (E)newBufferPeek(getNextBuffer(buffer, mask), index);
/*     */     }
/* 321 */     return (E)e;
/*     */   }
/*     */   
...
/* 361 */     AtomicReferenceArray<E> nextBuffer = (AtomicReferenceArray)LinkedAtomicArrayQueueUtil.lvElement(buffer, offset);
...
/* 432 */       return (E)newBufferPoll(nextBuffer, index);
/*     */     }
...
/* 448 */     if (e == JUMP) {
/* 449 */       return (E)newBufferPeek(getNextBuffer(buffer, mask), index);
/*     */     }
/* 451 */     return (E)e;
/*     */   }
/*     */   
...
/*     */ 
/*     */ 
/* 486 */         mask = this.producerMask;
/* 487 */         buffer = this.producerBuffer;
/*     */         
/*     */ 
/* 490 */         long batchIndex = Math.min(producerLimit, pIndex + 2 * batchSize);
/* 491 */         int result; if ((pIndex == producerLimit) || (producerLimit < batchIndex))
/* 492 */           result = offerSlowPath(mask, pIndex, producerLimit);
/* 493 */         switch (result) {
/*     */         case 1: 
/*     */           break;
/*     */         case 2: 
/* 497 */           return 0;
/*     */         case 3: 
/* 499 */           resize(mask, buffer, pIndex, s.get());
/* 500 */           return 1;
/*     */         
/*     */ 
/*     */         default: 
...
/* 539 */     for (int i = 0; 
/*     */         
/* 541 */         (i < limit) && ((m = relaxedPoll()) != null); i++) {
/* 542 */       c.accept(m);
/*     */     }
/* 544 */     return i;
/*     */   }
...
/*     */   }
/*     */   
/* 562 */   private void resize(long oldMask, AtomicReferenceArray<E> oldBuffer, long pIndex, E e) { int newBufferLength = getNextBufferSize(oldBuffer);
