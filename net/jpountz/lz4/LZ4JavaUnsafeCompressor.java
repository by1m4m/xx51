...
/*     */           }
/*     */           
/*  54 */           int h = LZ4Utils.hash64k(UnsafeUtils.readInt(src, sOff));
...
/*     */         
/*     */ 
/*  60 */         int excess = LZ4UnsafeUtils.commonBytesBackward(src, ref, sOff, srcOff, anchor);
/*  61 */         sOff -= excess;
/*  62 */         ref -= excess;
/*     */         
/*     */ 
...
/*     */         
/*  74 */         if (runLen >= 15) {
/*  75 */           UnsafeUtils.writeByte(dest, tokenOff, 240);
/*  76 */           dOff = LZ4UnsafeUtils.writeLen(runLen - 15, dest, dOff);
/*     */         } else {
/*  78 */           UnsafeUtils.writeByte(dest, tokenOff, runLen << 4);
/*     */         }
/*     */         
/*     */ 
...
/*  91 */           sOff += 4;
/*  92 */           ref += 4;
/*  93 */           int matchLen = LZ4UnsafeUtils.commonBytes(src, ref, sOff, srcLimit);
...
/*     */ 
/* 100 */           if (matchLen >= 15) {
/* 101 */             UnsafeUtils.writeByte(dest, tokenOff, UnsafeUtils.readByte(dest, tokenOff) | 0xF);
/* 102 */             dOff = LZ4UnsafeUtils.writeLen(matchLen - 15, dest, dOff);
/*     */           } else {
...
/*     */           }
/*     */           
/* 114 */           UnsafeUtils.writeShort(hashTable, LZ4Utils.hash64k(UnsafeUtils.readInt(src, sOff - 2)), sOff - 2 - srcOff);
/*     */           
/*     */ 
/* 117 */           int h = LZ4Utils.hash64k(UnsafeUtils.readInt(src, sOff));
...
/*     */         }
/*     */         
/* 178 */         int h = LZ4Utils.hash(UnsafeUtils.readInt(src, sOff));
/* 179 */         ref = UnsafeUtils.readInt(hashTable, h);
/* 180 */         back = sOff - ref;
/* 181 */         UnsafeUtils.writeInt(hashTable, h, sOff);
/* 182 */       } while ((back >= 65536) || (!LZ4UnsafeUtils.readIntEquals(src, ref, sOff)));
/*     */       
/*     */ 
/* 185 */       int excess = LZ4UnsafeUtils.commonBytesBackward(src, ref, sOff, srcOff, anchor);
/* 186 */       sOff -= excess;
/* 187 */       ref -= excess;
/*     */       
/*     */ 
...
/*     */       
/* 199 */       if (runLen >= 15) {
/* 200 */         UnsafeUtils.writeByte(dest, tokenOff, 240);
/* 201 */         dOff = LZ4UnsafeUtils.writeLen(runLen - 15, dest, dOff);
/*     */       } else {
/* 203 */         UnsafeUtils.writeByte(dest, tokenOff, runLen << 4);
/*     */       }
/*     */       
/*     */ 
...
/*     */ 
/* 216 */         sOff += 4;
/* 217 */         int matchLen = LZ4UnsafeUtils.commonBytes(src, ref + 4, sOff, srcLimit);
...
/*     */ 
/* 224 */         if (matchLen >= 15) {
/* 225 */           UnsafeUtils.writeByte(dest, tokenOff, UnsafeUtils.readByte(dest, tokenOff) | 0xF);
/* 226 */           dOff = LZ4UnsafeUtils.writeLen(matchLen - 15, dest, dOff);
/*     */         } else {
...
/*     */         }
/*     */         
/* 238 */         UnsafeUtils.writeInt(hashTable, LZ4Utils.hash(UnsafeUtils.readInt(src, sOff - 2)), sOff - 2);
/*     */         
/*     */ 
/* 241 */         int h = LZ4Utils.hash(UnsafeUtils.readInt(src, sOff));
/* 242 */         ref = UnsafeUtils.readInt(hashTable, h);
/* 243 */         UnsafeUtils.writeInt(hashTable, h, sOff);
/* 244 */         back = sOff - ref;
/*     */         
/* 246 */         if ((back >= 65536) || (!LZ4UnsafeUtils.readIntEquals(src, ref, sOff))) {
/*     */           break;
/*     */         }
/*     */         
/* 250 */         tokenOff = dOff++;
...
/*     */           }
/*     */           
/* 356 */           UnsafeUtils.writeShort(hashTable, LZ4Utils.hash64k(ByteBufferUtils.readInt(src, sOff - 2)), sOff - 2 - srcOff);
/*     */           
/*     */ 
...
/*     */         }
/*     */         
/* 486 */         UnsafeUtils.writeInt(hashTable, LZ4Utils.hash(ByteBufferUtils.readInt(src, sOff - 2)), sOff - 2);
/*     */         
/*     */ 
