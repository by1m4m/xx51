...
/*     */   {
/*  23 */     UnsafeUtils.checkRange(src, srcOff, srcLen);
/*  24 */     UnsafeUtils.checkRange(dest, destOff, destLen);
/*     */     
/*  26 */     if (destLen == 0) {
/*  27 */       if ((srcLen != 1) || (UnsafeUtils.readByte(src, srcOff) != 0)) {
...
/*     */     for (;;)
/*     */     {
/*  42 */       int token = UnsafeUtils.readByte(src, sOff) & 0xFF;
/*  43 */       sOff++;
/*     */       
/*     */ 
...
/*  47 */       if (literalLen == 15) {
/*  48 */         byte len = -1;
/*  49 */         while ((sOff < srcEnd) && ((len = UnsafeUtils.readByte(src, sOff++)) == -1)) {
/*  50 */           literalLen += 255;
/*     */         }
...
/*     */         }
/*     */         
/*  64 */         LZ4UnsafeUtils.safeArraycopy(src, sOff, dest, dOff, literalLen);
/*  65 */         sOff += literalLen;
...
/*     */       
/*     */ 
/*  71 */       LZ4UnsafeUtils.wildArraycopy(src, sOff, dest, dOff, literalLen);
/*  72 */       sOff += literalLen;
/*  73 */       dOff = literalCopyEnd;
/*     */       
/*     */ 
/*  76 */       int matchDec = UnsafeUtils.readShortLE(src, sOff);
/*  77 */       sOff += 2;
...
/*  85 */       if (matchLen == 15) {
/*  86 */         byte len = -1;
/*  87 */         while ((sOff < srcEnd) && ((len = UnsafeUtils.readByte(src, sOff++)) == -1)) {
/*  88 */           matchLen += 255;
/*     */         }
...
/*     */         }
/* 100 */         LZ4UnsafeUtils.safeIncrementalCopy(dest, matchOff, dOff, matchLen);
/*     */       } else {
/* 102 */         LZ4UnsafeUtils.wildIncrementalCopy(dest, matchOff, dOff, matchCopyEnd);
/*     */       }
